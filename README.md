...
> [!IMPORTANT]
> **Бесплатный тариф Gemini и модели.** Модель `gemini-2.5-pro` на **бесплатном** ключе недоступна (квота = 0), поэтому решатель капчи по умолчанию настроен на `gemini-2.5-flash` (`IMAGE_CLASSIFIER_MODEL`, `SPATIAL_POINT_REASONER_MODEL`, `SPATIAL_PATH_REASONER_MODEL` в `.env`) — так бесплатный ключ работает «из коробки». Если у вас платный (с биллингом) ключ, можете переключить эти переменные обратно на `gemini-2.5-pro` для более точного решения капчи.
